                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                    <div>
                      <div className="relative">
                        ...
                        <p className="text-clay-600">Les artisans derrière chaque expérience unique chez Zafran</p>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                        {aboutData?.data?.team?.map((member) => (
                            <div key={member.id} className="text-center">
                              <div className="aspect-[3/4] rounded-lg overflow-hidden mb-4 shadow">
                                <img
                                    src={`http://localhost:1337${member.image?.[0]?.formats?.thumbnail?.url}`}
                                    alt={member.name}
